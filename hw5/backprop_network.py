...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, learning_rate,
        ...
        plt.plot(train_acc , marker="o", label="accuracy on the training data")
        plt.plot(test_acc, marker="*", label="accuracy on the test data")
        plt.plot(train_loss, marker="+", label="training loss")
        plt.xlabel("Epoch")
        plt.ylabel("accuracy / loss")
        plt.legend()
        #plt.show()
        #D
        plt.plot(db0,marker="o",label="layer 0")
        plt.plot(db1, marker="o", label="layer 1")
        plt.plot(db2, marker="o", label="layer 2")
        plt.plot(db3, marker="o", label="layer 3")
        plt.plot(db4, marker="o", label="layer 4")
        plt.xlabel("Epoch")
        plt.ylabel("the gradient euclidean norm")
        plt.legend()
        #plt.show()
    ...
    def update_mini_batch(self, mini_batch, learning_rate):
        ...
        #D
        db0.append(np.linalg.norm(nabla_b[0])/len(mini_batch))
        db1.append(np.linalg.norm(nabla_b[1])/len(mini_batch))
        db2.append(np.linalg.norm(nabla_b[2])/len(mini_batch))
        db3.append(np.linalg.norm(nabla_b[3])/len(mini_batch))
        db4.append(np.linalg.norm(nabla_b[4])/len(mini_batch))
    ...
    def backprop(self, x, y):
        ...
        db[-1] = self.loss_derivative_wr_output_activations(z_s[-1],y)  # the derivative of last layer
        # start brack propagation
        for i in reversed(range(len(db) - 1)):
            db[i] = np.dot(self.weights[i + 1].T, db[i + 1]) * sigmoid_derivative(z_s[i])
        dw = [d.dot(a_s[i].T) for i, d in enumerate(db)]
        return db, dw
