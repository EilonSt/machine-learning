...
class KerasMnist(object):
    ...
    def build_model_no_skip(self):
        ...
        self.model.add(Dense(self.hidden_layer_dims[0], activation='relu', input_dim=self.input_dim))
    ...
    def build_model_skip(self):
        ...
        x_t=Dense(self.hidden_layer_dims[0])(x)
        x_shortcut = x_t
        Activation('relu')(x_t)
        for i in range(1, len(self.hidden_layer_dims)):
            if ((i - 1) % self.skips != 0) or (i == 1):
                x_t = Dense(self.hidden_layer_dims[i])(x_t)
            else:
                joinedTensor = keras.layers.add([x_shortcut, x_t])
                x_shortcut = x_t
                x_t = Dense(self.hidden_layer_dims[i], activation='relu')(joinedTensor)
            Activation('relu')(x_t)
        out = Dense(self.num_classes, activation='softmax')(x_t)
        ### YOUR CODE ENDS HERE
